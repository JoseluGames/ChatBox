...
public class ChatBoxEventHandler {
	...
	public void onPlayerTickEvent(PlayerTickEvent event){
		if(!ChatBoxMain.haveWarnedVersionOutOfDate && event.player.world.isRemote && !ChatBoxMain.versionChecker.isLatestVersion()){
			...
			for(int i = 1; i < ChatBoxMain.versionChecker.getLatestVersionInfo().size(); i++){
				event.player.sendMessage(new TextComponentString("- " + ChatBoxMain.versionChecker.getLatestVersionInfo().get(i)).setStyle(new Style().setColor(TextFormatting.YELLOW)));
			}
